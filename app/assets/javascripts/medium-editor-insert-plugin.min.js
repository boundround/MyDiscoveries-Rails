...
! function(a) {
    ...
    this.MediumInsert = this.MediumInsert || {}, this.MediumInsert.Templates = this.MediumInsert.Templates || {}, this.MediumInsert.Templates["src/js/templates/core-buttons.hbs"] = Handlebars.template({
            ...
            main: function(a, b, c, d, e) {
                ...
                old_img = $(figure).find("img").attr("src");
                position = $(figure).find("img");
                old_format = old_img.split("/")[1].split(";")[0];
                ...
                var width = img.onload().w;
                    height = img.onload().h;

                if ( width > 1200 ) {
                    nwh = (1200/width)*height;
                    ...
                    function resizeBase64Img(base64, width, height) {
                        ...
                        $("<img/>").attr("src", base64).load(function() {
                            ...
                            deferred.resolve($("<img/>").attr("src", canvas.toDataURL("image/"+old_format, 1)));
                        });

                        return [deferred.promise(), canvas.toDataURL("image/"+old_format, 1), base64];    
                    }

                    var test = resizeBase64Img(old_img, 1200, nwh, position);
                    var new2 = test[1] 
                    var lama = test[2]
                    test[0].then(function(newImg){

                        new_img = $($(newImg)[0]).attr("src");

                        $.each($('figure'), function(i, v){
                            if ( $(v).find("img").attr("src") == lama ) {
                                $(v).find("img").attr("src", new_img);
                            }
                        });
        ...
        function(a, b, c, d) {
            ...
            f.prototype.init = function() {
                ...
                var e, f, g, h;
                d = d || 0, e = c.createRange(), f = b.getSelection(), g = a.get(0), g.childNodes.length || (h = c.createTextNode(" "), g.appendChild(h)), e.setStart(g.childNodes[0], d), e.collapse(!0), f.removeAllRanges(), f.addRange(e)
        ...
        function(a, b, c, d) {
            ...
            e.prototype.init = function() {
                ...
                var c, d;
                a(".medium-insert-embeds-active").length || (c = b.originalEvent.clipboardData.getData("text"), d = new RegExp("^(http(s?):)?//", "i"), d.test(c) && (this.options.oembedProxy ? this.oembed(c, !0) : this.parseUrl(c, !0)))
                ...
                a.support.cors = !0, a.ajax({
                    ...
                    success: function(b) {
                        var f = b && b.html;
                        return b && !f && "photo" === b.type && b.url && (f = '<img src="' + b.url + '" alt="">'), f ? void(d ? a.proxy(e, "embed", f, c)() : a.proxy(e, "embed", f)()) : void a.proxy(e, "convertBadEmbed", c)()
                    },
                ...
                c = this.$el.find(".medium-insert-embeds-active"), e = a(f), c.before(e), c.remove(), e.html(b), d = a(f), e.after(d), this.core.triggerInput(), this.core.moveCaret(d)
            }, e.prototype.selectEmbed = function(b) {
                var c, d = this;
                this.core.options.enabled && (c = a(b.target).hasClass("medium-insert-embeds") ? a(b.target) : a(b.target).closest(".medium-insert-embeds"), c.addClass("medium-insert-embeds-selected"), setTimeout(function() {
                ...
                var b, c, d, e, f, g = this.$el.find(".medium-insert-embeds-selected"),
                    h = !1;
                0 !== g.length && (e = this.core.getEditor(), f = e.options.elementsContainer || "body", a(f).append(this.templates["src/js/templates/embeds-toolbar.hbs"]({
                    styles: this.options.styles,
                    actions: this.options.actions
                }).trim()), b = a(".medium-insert-embeds-toolbar"), c = a(".medium-insert-embeds-toolbar2"), d = g.offset().top - b.height() - 8 - 2 - 5, 0 > d && (d = 0), b.css({
                    top: d,
                    left: g.offset().left + g.width() / 2 - b.width() / 2
                }).show(), c.css({
                    top: g.offset().top + 2,
                    left: g.offset().left + g.width() - c.width() - 4
                }).show(), b.find("button").each(function() {
                    g.hasClass("medium-insert-embeds-" + a(this).data("action")) && (a(this).addClass("medium-editor-button-active"), h = !0)
                }), h === !1 && b.find("button").first().addClass("medium-editor-button-active"))
        ...
        function(a, b, c, d, e) {
            ...
            f.prototype.init = function() {
                ...
                return f.click(), e = this, this.options.preview && c.context ? (d = this.getDOMImage(), d.onload = function() {
                    ...
                    }), f.addClass("medium-insert-images-grid"), this.options.styles.grid.added && this.options.styles.grid.added(f)), this.options.preview ? c.submit() : this.options.uploadCompleted && this.options.uploadCompleted(c.context, c)),
                    this.core.triggerInput(), c.context
            }, f.prototype.getDOMImage = function() {
                return new b.Image
            }, f.prototype.selectImage = function(b) {
                var c, d = this;
                this.core.options.enabled && (c = a(b.target), this.$currentImage = c, this.$el.blur(), c.addClass("medium-insert-image-active"), c.closest(".medium-insert-images").addClass("medium-insert-active"), setTimeout(function() {
                ...
                var e, f, g, h, i, j, k, l, m, n, o = [],
                    p = this.$el.find(".medium-insert-image-active");
                if ((8 === d.which || 46 === d.which) && (p.length && o.push(p), g = b.getSelection(), g && g.rangeCount && (h = g.getRangeAt(0), i = h.commonAncestorContainer, k = "#text" === i.nodeName ? a(i).parent() : a(i), j = MediumEditor.selection.getCaretOffsets(i).left, 8 === d.which && 0 === j ? l = k.prev() : 46 === d.which && j === k.text().length && (l = k.next()), l && l.hasClass("medium-insert-images") && o.push(l.find("img")), m = MediumEditor.selection.getSelectionHtml(c), m && a("<div></div>").html(m).find(".medium-insert-images img").each(function() {
                        o.push(a(this))
                    })), o.length)) {
                    for (n = 0; n < o.length; n++) this.deleteFile(o[n].attr("src")), e = o[n].closest(".medium-insert-images"), o[n].closest("figure").remove(), 0 === e.find("figure").length && (f = e.next(), (f.is("p") === !1 || "" !== f.text()) && (f = a(this.templates["src/js/templates/core-empty-line.hbs"]().trim()), e.before(f)), e.remove());
                    this.core.hideAddons(), !m && f && (d.preventDefault(), this.core.moveCaret(f)), a(".medium-insert-images-toolbar, .medium-insert-images-toolbar2").remove(), this.core.triggerInput()
                }
            }, f.prototype.deleteFile = function(b) {
                this.options.deleteScript && a.ajax(a.extend(!0, {}, {
                ...
                null !== this.$currentImage && (c = a(b.target).is("button") ? a(b.target) : a(b.target).closest("button"), d = c.closest("li"), e = d.closest("ul"), f = e.find("li"), g = this.$el.find(".medium-insert-active"), c.addClass("medium-editor-button-active"), d.siblings().find(".medium-editor-button-active").removeClass("medium-editor-button-active"), f.find("button").each(function() {
                ...
                var c, d;
                null !== this.$currentImage && (c = a(b.target).is("button") ? a(b.target) : a(b.target).closest("button"), d = this.options.actions[c.data("action")].clicked, d && d(this.$el.find(".medium-insert-image-active")), this.core.hideButtons(), this.core.triggerInput())
