...
#    @videos = Video.includes(:place => :area).order('places.display_name ASC, areas.display_name ASC').paginate(:page => params[:page])
  ...
  private
    ...
    def video_params
      params.require(:video).permit(:vimeo_id, :area_id, :title, :description, :place_id, :priority, :vimeo_thumbnail, :status, :country_include, :customer_approved, :customer_review, :approved_at, :_destroy)
    end
