    ...
    if !childrens.blank?
      childrens.each do |child_place|
        if child_place.itemable_type == 'Country'
          child_place_item = child_place.itemable.blank?? [] : child_place.itemable.childrens.select{|childplace| childplace.itemable_type == 'Place'}
      ...
      childrens_collect.each do |place|
        unless place.blank?
          ...
          data_objs['#country'] = place.parent.parentable.display_name
          ...
          place.childrens.last(3).each do |place_child|
            if place_child.itemable.present?
              ...
              data_child_objs = {}
              data_child_objs['@country_child'] = place_item_child.parent.parentable.display_name
