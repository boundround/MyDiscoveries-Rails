    ...
    init: function(opts) {
        ...
        eventBus.on(jigsaw.Events.JIGSAW_COMPLETE, function(){
            ...
            // just show time
                ...
                self.ui.show_time();
//                var t=~~((new Date().getTime()-self.ui.ini)/1000);window.location='puzzle:completed:'+t;
            }
        });
        ...
        function resizeView() {
            ...
            cv1.height = document.documentElement.clientHeight;
            self.ce.redraw();
...
function jigsaw_events(ce, eventBus, rotate) {
    ...
    Util.addEvent(ce.cv,"tap",function(e){if(rotate&&ce.focused){ce.focused.rotation=(ce.focused.rotation+45)%360;ce.redraw();}e.preventDefault();});
