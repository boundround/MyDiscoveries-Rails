...
def pathtobundlejpggamefile(bundlename,filename,gamedirectory,gamebundle) :
  if gamebundle == "Main" :
    fullurl = urllib.quote(pathtobundlejpgfilefrommainbundlegame(bundlename,filename,gamedirectory), safe="%/:=&?~#+!$,;'@()*[]")
    return fullurl
...
for area in areas :
  ...
  for place in area['places'] :
    ...
    qs = []
    jinfo['questions'] = []
    jinfo['questions'].append("What was the best thing?")
    jinfo['questions'].append("What is your funniest memory?")
    jinfo['questions'].append("What did you eat that you loved/didn't like?")
    jinfo['questions'].append("What was the weirdest thing you saw?")
    jinfo['questions'].append("Your Story:")
    jinfo['questions'].append("AA_Rating")
    place['journal_info'] = jinfo
  ...
  for place in area['places'] :
    ...
    if place['Game'] == 'wordsearch' :
      ...
      #PARAMETER ORDER IS CRITICAL!
      jsonRep['Parameters']=['Photo','Pieces']
      jsonRep['Pieces'] = "12"
