...
#          puts i
        ...
        if download_url != ''
          ...
          download_video(download_url, video['name'])
          videos_total += 1
        end
      end
      current_page += 1
  ...
  def download_video(url, vimeo_id)
    vid_file = "'./videos/hdvideos/#{vimeo_id}.mp4'"
  ...
  def get_thumbs
    ...
    while current_page <= pages
      ...
      response.body['data'].each do |video|
#        puts video['pictures']
        download_url = video['pictures'][0]['link']
        vimeo_id = video['uri'].match(/\d+$/)[0]
        puts video['uri']
        puts "downloading #{vimeo_id}"
#        download_video_thumbs(download_url, vimeo_id)
        download_video_thumbs(download_url, video['name'])
        thumbs_total += 1
      end
      current_page += 1
    end
  ...
  def download_video_thumbs(url, vimeo_id)
    thumb_file = "'./videos/hdvideos/thumbs/#{vimeo_id}.jpg'"
